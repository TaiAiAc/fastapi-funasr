...
async def analyze_voice(
    ...
    try:
            ...
            try:
                ...
                if file_ext == "m4a" or "Format not recognised" in str(sf_error):
                    ...
                    audio_segment = AudioSegment.from_file(
                        io.BytesIO(audio_data), format=file_ext
                    )
                    ...
                    sr = sample_rate
                    info(
                        f"使用pydub处理成功，采样率: {sr}Hz, 音频长度: {len(data)}样本"
                    )
                    valid_length = len(data) * 1000 / sr  # 转换为毫秒
                    info(
                        f"音频数据范围: [{data.min():.6f}, {data.max():.6f}]"
                    )  # 添加范围日志
                else:
                    raise sf_error
        ...
        # 创建VAD流进行分析
        vad_stream = vad_service.create_stream(**analyze_params)
        ...
        # 将音频分割成小块进行处理
        chunk_samples = int(
            vad_stream.chunk_duration_ms * sample_rate / 1000
        )  # 600ms → 9600
        ...
        info(
            f"音频分析完成，耗时: {process_time:.2f}秒，检测到 {len(all_segments)} 个语音段"
        )

        # 计算语音总时长
        total_voice_duration = (
            sum(end - start for start, end in all_segments) if all_segments else 0
        )
        ...
        return JSONResponse(
            ...
            content={
                ...
                "total_voice_duration": round(
                    total_voice_duration, 2
                ),  # 语音总时长(ms)
                "voice_ratio": (
                    round(total_voice_duration / audio_duration * 100, 2)
                    if audio_duration > 0
                    else 0
                ),  # 语音占比(%)
